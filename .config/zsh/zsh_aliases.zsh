...
alias 'battery'='if [ -f /sys/class/power_supply/BAT1/charge_now ] ; then ((echo "Battery: " ; ((echo "scale=3;" ; cat /sys/class/power_supply/BAT1/charge_now ; echo / ; cat /sys/class/power_supply/BAT1/charge_full) | tr -d '"'"'\n'"'"' ; echo "*100") | bc | tr -d '"'"'\n'"'"' | perl -pe '"'"'s/(\d*)((\.|(\.\d+?))0*)?$/\1\4/gp'"'"') | tr -d '"'"'\n'"'"'; echo "%") else echo "No battery" ; fi'
